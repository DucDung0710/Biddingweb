package com.bidding.controller.wallet;
...
public class WalletSellerController implements Initializable {
    ...
    @FXML private HBox navTransactionHistory;
    @FXML private HBox navNotification;
    @FXML private HBox btnLogout;

    private final WalletService walletService = new WalletService();
    private int currentUserId;
    private final double totalRevenue = 0;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        cmbDepositMethod.valueProperty().addListener((obs, oldVal, newVal) -> handleMethodChange(newVal));
    ...
    private void loadCurrentUser() {
        if (UserSession.getInstance().getLoggedInUser() != null) {
            ...
            lblUserRole.setText(UserSession.getInstance().getLoggedInUser().getRole());
        } else {
    ...
    private void setupDepositCombo() {
        cmbDepositMethod.setItems(FXCollections.observableArrayList("💳 Yêu cầu Admin nạp tiền"));
    }

    private void setupTxFilterCombo() {
        cmbTxFilter.setItems(FXCollections.observableArrayList("Tất cả", "Nạp tiền", "Rút tiền"));
    ...
    private void setupNavigation() {
        ...
        if (navTransactionHistory != null) {
            navTransactionHistory.setOnMouseClicked(e -> SceneManager.switchToSellerTransactionHistory());
        }
        if (navNotification != null) {
            navNotification.setOnMouseClicked(e -> SceneManager.switchToSellerNotifications());
        }
        if (btnLogout != null) {
    ...
    private void loadWalletData() {
        currentUserId = UserSession.getInstance().getLoggedInUser() != null ? UserSession.getInstance().getLoggedInUser().getId() : 0;
