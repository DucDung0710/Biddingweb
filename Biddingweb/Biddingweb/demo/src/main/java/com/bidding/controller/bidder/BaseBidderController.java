...
public abstract class BaseBidderController {
    ...
    protected void setupSidebarBehavior() {
        if (navDashboard != null) {
            navDashboard.setOnMouseClicked(e -> {
                SocketClient.getInstance().clearPushListener(); // Dọn dẹp luồng realtime trước khi chuyển màn
        ...
        if (navAuctionHistory != null) {
            navAuctionHistory.setOnMouseClicked(e -> {
                SocketClient.getInstance().clearPushListener(); // Dọn dẹp luồng realtime trước khi chuyển màn
                SceneManager.switchToAuctionHistory();
            });
        }
        if (navWallet != null) {
            navWallet.setOnMouseClicked(e -> {
                SocketClient.getInstance().clearPushListener(); // Dọn dẹp luồng realtime trước khi chuyển màn
                SceneManager.switchToBidderWallet();
            });
        }
        if (btnLogout != null) {
            btnLogout.setOnMouseClicked(e -> {
                SocketClient.getInstance().clearPushListener(); // Dọn dẹp luồng realtime trước khi đăng xuất
    ...
    private void showNotificationPopup() {
        try {
            ...
            popupStage.initModality(Modality.APPLICATION_MODAL); // Ngăn tương tác với màn hình chính khi đang mở popup
            ...
            // 4. Khi người dùng mở xem thông báo, ẩn badge số lượng đi
    ...
    protected void handleLogout() {
        SocketClient.getInstance().clearPushListener(); // Dọn dẹp trước khi thoát hoàn toàn
